  ...
  def __init__(self, *args):
    ...
    def _on_ready(self, event):
        if not isinstance(self.unit.status, BlockedStatus):
            self.external_idp_provider.create_provider(providers)
```
...
import json
import logging
from typing import Annotated, Iterator, Literal, Mapping, Union
...
class BaseProvider(BaseModel):
    ...
    secret_backend: Literal["relation", "secret", "vault"] = Field(default="relation")
    id: str | None = Field(
        default=None,
    ...
    jsonnet_mapper: str | None = Field(default=None)
    mapper_url: str | None = Field(default=None)
    relation_id: int | None = Field(default=None, exclude=True)
    ...
    @classmethod
    def deserialize_scope(cls, v: str | None) -> list[str]:
...
class ExternalIdpProvider(Object):
    ...
    def get_redirect_uri(self, relation_id: int | None = None) -> str | None:
    ...
    @staticmethod
    def validate_provider_config(configurations: list[Mapping]) -> Providers | None:
...
class ExternalIdpRequirer(Object):
    ...
    def get_providers_from_relation(self, relation: Relation) -> Providers | None:
        if not relation.app:
            return None
